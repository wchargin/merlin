...
let label_mismatch l tl = Label_mismatch (l, trace_copy tl)
let pattern_type_clash tl = Pattern_type_clash (trace_copy tl)
...
let wrong_name s1 t s2 p s3 l = Wrong_name (s1, deep_copy () t, s2, p, s3, l)
let undefined_method t s l = Undefined_method (deep_copy () t, s, l)
...
and type_expect ?in_function ?recarg env sexp ty_expected =
  ...
  let aux () =
    if ~!Typing_aux.relax_typer then
      ...
      let snap = Btype.snapshot () in
      try type_expect_ ?in_function ?recarg env sexp ty_expected
      with (Typetexp.Error _ | Error _) ->
        Btype.backtrack snap;
        Fluid.let' Typing_aux.relax_typer true
          (fun () -> type_relax ?in_function ?recarg env sexp ty_expected)
  in
...
and type_relax ?in_function ?recarg env sexp ty_expected =
  ...
  try
    let ty = newvar () in
    let exp = type_expect_ ?in_function ?recarg env sexp ty in
    try
